...
//#pragma omp parallel for reduction(+:totalusedtokey)
	...
	for (int k = 0; k < apnum[1]; k++) {
		cout << "apnum[1]'s aplist[" << k << "] = " << aplist[1][k] << endl;
	}
	for (int k = 0; k < apnum[2]; k++) {
		cout << "apnum[2]'s aplist[" << k << "] = " << aplist[2][k] << endl;
	}
	for (int k = 0; k < apnum[3]; k++) {
		cout << "apnum[3]'s aplist[" << k << "] = " << aplist[3][k] << endl;
	}
	cout << endl;
